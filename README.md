...
```go
import (
	"github.com/stainless-sdks/dodo-payments-go" // imported as dodopayments
)
```
...
```sh
go get -u 'github.com/stainless-sdks/dodo-payments-go@v0.0.1-alpha.0'
```
...
func main() {
	client := dodopayments.NewClient(
		...
		option.WithEnvironmentLiveMode(), // defaults to option.WithEnvironmentTestMode()
	)
...
See the [full list of request options](https://pkg.go.dev/github.com/stainless-sdks/dodo-payments-go/option).
...
We also provide a helper `dodopayments.FileParam(reader io.Reader, filename string, contentType string)`
...
We are keen for your feedback; please open an [issue](https://www.github.com/stainless-sdks/dodo-payments-go/issues) with questions, bugs, or suggestions.
