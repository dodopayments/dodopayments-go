...
func TestProductNewWithOptionalParams(t *testing.T) {
	...
	_, err := client.Products.New(context.TODO(), dodopayments.ProductNewParams{
		...
		CreditEntitlements: dodopayments.F([]dodopayments.ProductNewParamsCreditEntitlement{{
			CreditEntitlementID:          dodopayments.F("credit_entitlement_id"),
			CreditsAmount:                dodopayments.F("credits_amount"),
			CreditsReduceOverage:         dodopayments.F(true),
			Currency:                     dodopayments.F(dodopayments.CurrencyAed),
			...
			PreserveOverageAtReset:       dodopayments.F(true),
			PricePerUnit:                 dodopayments.F("price_per_unit"),
			ProrationBehavior:            dodopayments.F(dodopayments.ProductNewParamsCreditEntitlementsProrationBehaviorProrate),
			...
			TrialCredits:                 dodopayments.F("trial_credits"),
			TrialCreditsExpireAfterTrial: dodopayments.F(true),
		}}),
...
func TestProductUpdateWithOptionalParams(t *testing.T) {
	...
	err := client.Products.Update(
		...
		dodopayments.ProductUpdateParams{
			Addons:  dodopayments.F([]string{"string"}),
			BrandID: dodopayments.F("brand_id"),
			CreditEntitlements: dodopayments.F([]dodopayments.ProductUpdateParamsCreditEntitlement{{
				CreditEntitlementID:          dodopayments.F("credit_entitlement_id"),
				CreditsAmount:                dodopayments.F("credits_amount"),
				CreditsReduceOverage:         dodopayments.F(true),
				Currency:                     dodopayments.F(dodopayments.CurrencyAed),
				...
				PreserveOverageAtReset:       dodopayments.F(true),
				PricePerUnit:                 dodopayments.F("price_per_unit"),
				ProrationBehavior:            dodopayments.F(dodopayments.ProductUpdateParamsCreditEntitlementsProrationBehaviorProrate),
				RolloverEnabled:              dodopayments.F(true),
				RolloverPercentage:           dodopayments.F(int64(0)),
				RolloverTimeframeCount:       dodopayments.F(int64(0)),
				RolloverTimeframeInterval:    dodopayments.F(dodopayments.TimeIntervalDay),
